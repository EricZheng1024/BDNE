...
function [y_N,Y_P,n_model_solved,runtime,runtime_solver,runtime_k] = NPDA_MOIQP(H,f,A,b,lb,ub,Aeq,beq,min_finterval,p, ...
    filename, ...
    runtime0_k,options)
...
%% Pay-off table
[y_I, n_model_solved, runtime_solver, T, y_PT, I_PT] = payoff_table_MOIQP(H,f,A,b,lb,ub,Aeq,beq,p, ...
...
for k = 1 : p
    ...
    while ~isempty(L) && cputime-start_time_k < runtime0_k-runtime_pre/p
        ...
        % A_P = [A; f(k,:); f(C,:)];
        % b_P = [b; y_U(k); u_i-tol];
        Q_P = [{1/2*H{k}},    cellfun(@(H) 1/2*H, H(C), 'UniformOutput', false)];
        l_P = [f(k,:); f(C,:)];
        r_P = [y_U(k); u_i-tol];
        ...
        Opt = opti('qp',H_m,f_m', ...
            'qc', Q_P, l_P', r_P, ...
            'ineq',A,b, ...
            'eq',Aeq,beq,'bounds',lb,ub,'xtype',1:length(f_m),'options',options);
        ...
        switch exitflag
            case 1  % Optimal
                tmp = round(tmp);
                z_star = 1/2*(tmp')*H_m*tmp + f_m*tmp;  % 如果不准确，会使得R没有可行解
                % R
                % A_R = [A; f(k,:); f(C,:)];
                % b_R = [b; y_U(k); u_i-tol];
                % Aeq_R = [Aeq; f_m];
                ...
                % Q_R = [{1/2*H{k}},    cellfun(@(H) 1/2*H, H(C), 'UniformOutput', false),    1/2*H_m];
                % l_R = [f(k,:); f(C,:); f_m];
                % r_R = [y_U(k); u_i-tol; z_star];
                Q_R = [Q_P, 1/2*H_m];
                l_R = [l_P; f_m];
                r_R = [r_P; z_star];
                ...
                switch exitflag
                    ...
                    case -1
                        ...
                        if isempty(Aeq)
                            ...
                            flag_Aeq = abs(Aeq*tmp-beq)<=0;
                        end
                        flag_Q_R = zeros(length(Q_R),1);
                        for i = 1 : length(Q_R)
                            flag_Q_R(i) = 1/2*(tmp')*Q_R{i}*tmp+l_R(i,:)*tmp - r_R(i) <= 0;
                        end
                        if all([flag_Q_R; flag_A; flag_Aeq])  % 可行域中只有一个点时，scip也似乎失效
                            x_star = tmp;
                        else
                            error('Unknown error (-2).')
                        end
                ...
                f_bar_x_star = cellfun(@(H) 1/2*(x_star')*H*x_star,H(C))' + f(C,:)*x_star;
