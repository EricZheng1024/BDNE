...
function [y_N,Y_P,n_model_solved,runtime,runtime_solver,runtime_k] = BDNE_MOILP(f,A,b,lb,ub,Aeq,beq,min_finterval,p, ...
    filename, ...
    runtime0_k,options)
...
for k = 1 : p
    ...
    while ~isempty(L) && cputime-start_time_k < runtime0_k-runtime_pre/p
        ...
        A_bd = [[zeros(size(A,1),1) A]; [-ones(p-1,1) left_tilde./z_cusp_tilde]];  % 直接套入表达式，减少误差
